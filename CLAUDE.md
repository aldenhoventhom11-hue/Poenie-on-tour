...
2. **Versienummer bumpen bij elke oplevering**: zoek `id="home-version"` (tekst zoals `versie 4.4`) en verhoog hem. Thom checkt dit nummer op zijn iPhone om cache-problemen te herkennen. Huidige versie: **4.4**.
...
- **Stem-wachtrij + ducking**: persoonlijke karakter-geluidjes (alle ingesproken stemmen: quotes, actie-clips, chick-mini-game) spelen NOOIT tegelijk maar via een FIFO-wachtrij (`voiceQueue`/`voicePlaying`/`enqueueVoice`/`playNextVoice`). Elke starter krijgt een `done`-callback (audio `ended`/`error` of TTS `u.onend`) + 15s-veiligheidstimer. Zolang de wachtrij loopt zakt `bgAudio.volume` subtiel van `NORMAL_MUSIC_VOL` (0.55) naar `DUCK_MUSIC_VOL` (0.30); leeg → terug. SFX-blips (bier/sprong/gold) lopen NIET via de wachtrij en dempen dus niet. Alle stem-paden (`speakLine`/`speakCharacter`/`playClip`/`playClipLoud`) gebruiken `queueAudio()` of `enqueueVoice()`.
