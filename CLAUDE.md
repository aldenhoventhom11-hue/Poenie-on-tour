...
2. **Versienummer bumpen bij elke oplevering**: zoek `id="home-version"` (tekst zoals `versie 3.6`) en verhoog hem. Thom checkt dit nummer op zijn iPhone om cache-problemen te herkennen. Huidige versie: **3.6**.
...
- **Character-items** (zeldzame pickups, kans 0.32, max 3/run, via `CHAR_ITEM`): bram boomstam (`logHeld/logFly`), jarno poepspray (`sprayHeld/sprayAnim`), olle hamburger (automatisch, `burgerT`), julian gewichten (`julianAct` met fases reach/carry/throw; `drawCarriedChaser`), timo wok (`wokFly` → `burnT=300`, chaser brandt maar blijft volgen), daan rode pil (`pillAnim` → `rageT=600`, onkwetsbaar), thom XXL-bier (`drinkT=120` → `freezeT=300`: update() bevriest de hele wereld vroeg met een return, Thom danst). Eén generieke `#item-btn` (emoji per item), gestapeld onder de ⚡-knop links samen met `#log-btn`/`#spray-btn`.
...
- **Mobiel**: `IS_TOUCH` → dynamische speelveldbreedte `VW` (800–1280, past zich aan de schermverhouding aan); grote onzichtbare raakzones `.ctrl-zone` (32% breed × 58% hoog) voor springen/sliden; menu-panelen schalen zichzelf passend via `fitPanels()` (nooit scrollen); iOS-fullscreen via "Zet op beginscherm"-uitleg (`#ios-tip`, ook op `#rotate-overlay`).
- **Gepakt-cinematic**: bij `endRun()` slow-zoom + letterbox-balken + zwaailicht-tint (`cineStart`), daarna pas de banner.
- **Leaderboard**: gelaagd in `loadBoard`/`saveBoard`: npoint.io (als `LB_NPOINT_ID` is ingevuld) → `window.storage` (Claude-preview) → `localStorage`. `#lb-source` toont de actieve bron.

## Openstaande punten

- **Gedeeld online leaderboard activeren**: Thom moet op https://www.npoint.io een document maken met inhoud `{"scores":[]}`, opslaan, en de code uit de URL plakken op de regel met `VUL HIER JE NPOINT-CODE IN` (variabele `LB_NPOINT_ID`). Daarna deelt iedereen via de link één leaderboard.
